	...
	for (int i = 0; i < 5; i++) { //Mode
		for (int j = 0; j < 6; j++) { //RA
			for (int k = 0; k < 3; k++) { //Ma
				...
				threads[2 * k] = std::thread(Simulation, mode, phases, particles, dimensions, box_length, numbers[2 * k], numbers[j], delta_t, 0, times);
				threads[2 * k + 1] = std::thread(Simulation, mode, phases, particles, dimensions, box_length, numbers[2 * k + 1], numbers[j], delta_t, 0, times);
			}
			for (int l = 0; l < 6; l++) {
				threads[l].join();
			}
